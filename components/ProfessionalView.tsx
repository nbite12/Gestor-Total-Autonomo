...
import { Income, Expense, Attachment, UserSettings, MoneyLocation, InvestmentGood } from '../types';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Select label="Tipo de IVA (%)" name="vatRate" value={formData.vatRate} onChange={handleChange}>
                    ...
                    <option value="10">10% (Reducido)</option>
                    <option value="4">4% (Superreducido)</option>
                    <option value="0">0% (Exento/No aplicable)</option>
                </Select>
            </div>
...
const InvestmentGoodForm: React.FC<{ onClose: () => void; goodToEdit?: InvestmentGood | null; }> = ({ onClose, goodToEdit }) => {
    const { setData } = useContext(AppContext)!;
    const [formData, setFormData] = useState<Partial<InvestmentGood>>({
        id: goodToEdit?.id || `inv-${Date.now()}`,
        purchaseDate: goodToEdit?.purchaseDate ? formatDateForInput(goodToEdit.purchaseDate) : new Date().toISOString().split('T')[0],
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-4">
                <Button type="button" variant="secondary" onClick={onClose}>Cancelar</Button>
                <Button type="submit">{goodToEdit ? 'Guardar Cambios' : 'Añadir Bien'}</Button>
            </div>
        </form>
    );
};
...
const ProfessionalView: React.FC = () => {
    ...
    const [investmentToEdit, setInvestmentToEdit] = useState<InvestmentGood | null>(null);
    const [isAiImportModalOpen, setAiImportModalOpen] = useState(false);

    // Handlers
    // FIX: Changed income type to Partial<Income> to match state and allow partial data from AI import.
    const handleOpenIncomeModal = (income?: Partial<Income>) => { setIncomeToEdit(income || null); setIncomeModalOpen(true); };
    // FIX: Changed expense type to Partial<Expense> for consistency and to match state.
    const handleOpenExpenseModal = (expense?: Partial<Expense>) => { setExpenseToEdit(expense || null); setExpenseModalOpen(true); };
    const handleOpenInvestmentModal = (good?: InvestmentGood) => { setInvestmentToEdit(good || null); setInvestmentModalOpen(true); };
    ...
    const handleAnalysisComplete = (extractedData: Partial<Income> | Partial<Expense>, type: 'income' | 'expense', file: File) => {
        ...
        reader.onload = () => {
            ...
            if (type === 'income') {
                ...
                setExpenseToEdit(dataWithAttachment);
        ...
        <div className="space-y-6">
            <div className="flex flex-wrap justify-between items-center gap-4">
                ...
                <div title={!data.settings.geminiApiKey ? "Configura tu API Key de Gemini en Ajustes para activar" : "Importar factura con IA"}>
                    <Button onClick={() => setAiImportModalOpen(true)} disabled={!data.settings.geminiApiKey}>
            ...
            <Modal isOpen={isIncomeModalOpen} onClose={() => setIncomeModalOpen(false)} title={incomeToEdit?.id ? "Editar Factura Emitida" : "Nueva Factura Emitida"}>
                <IncomeForm onClose={() => setIncomeModalOpen(false)} incomeToEdit={incomeToEdit} />
            </Modal>
            <Modal isOpen={isExpenseModalOpen} onClose={() => setExpenseModalOpen(false)} title={expenseToEdit?.id ? "Editar Factura Recibida" : "Nueva Factura Recibida"}>
                <ExpenseForm onClose={() => setExpenseModalOpen(false)} expenseToEdit={expenseToEdit} />
            </Modal>
            <Modal isOpen={isInvestmentModalOpen} onClose={() => setInvestmentModalOpen(false)} title={investmentToEdit ? "Editar Bien de Inversión" : "Nuevo Bien de Inversión"}>
                <InvestmentGoodForm onClose={() => setInvestmentModalOpen(false)} goodToEdit={investmentToEdit} />
            </Modal>
        </div>
    );
};
...
const LibrosRegistroView: React.FC<{
    // FIX: Allow partial/undefined objects to enable creating new items and handling partial data.
    onEditIncome: (income?: Partial<Income>) => void;
    onEditExpense: (expense?: Partial<Expense>) => void;
    onEditInvestment: (good?: InvestmentGood) => void;
        ...
        <div className="space-y-8">
            ...
            <Card>
                <div className="flex justify-between items-center mb-4">
                    ...
                    {/* FIX: Call handler without arguments to create a new item. */}
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {incomes.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(inc => (
                                <tr key={inc.id} className="border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600">
                                    <td className="px-4 py-2">{formatDate(inc.date)}</td>
                                    <td className="px-4 py-2">{inc.invoiceNumber}</td>
                                    <td className="px-4 py-2">{inc.clientName}</td>
                                    <td className="px-4 py-2 text-right">{formatCurrency(inc.baseAmount)}</td>
                                    <td className="px-4 py-2 text-right">{formatCurrency(getCuotaIVA(inc.baseAmount, inc.vatRate))} ({inc.vatRate}%)</td>
                                    <td className="px-4 py-2 text-right">{formatCurrency(getCuotaIRPF(inc.baseAmount, inc.irpfRate))} ({inc.irpfRate}%)</td>
                                    <td className="px-4 py-2 text-right font-bold">{formatCurrency(getTotalFacturaEmitida(inc))}</td>
                                    <td className="px-4 py-2 flex justify-center gap-1">
                                        <Button size="sm" variant="ghost" onClick={() => onEditIncome(inc)} title="Editar"><Icon name="pencil" className="w-4 h-4" /></Button>
                                        <Button size="sm" variant="ghost" onClick={() => onDelete('income', inc.id)} title="Eliminar"><Icon name="trash" className="w-4 h-4 text-red-500" /></Button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    ...
                    <table className="w-full text-sm">
                         <thead className="text-xs text-slate-700 uppercase bg-slate-50 dark:bg-slate-700 dark:text-slate-400">
                            <tr>
                                ...
                                <th className="px-4 py-2 text-right">IVA</th>
                                <th className="px-4 py-2 text-right">Total</th>
                                <th className="px-4 py-2 text-center">Deducible</th>
                                <th className="px-4 py-2 text-center">Acciones</th>
                            </tr>
                        </thead>
                        <tbody>
                            {expenses.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(exp => (
                                <tr key={exp.id} className="border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600">
                                    <td className="px-4 py-2">{formatDate(exp.date)}</td>
                                    <td className="px-4 py-2">{exp.providerName}</td>
                                    <td className="px-4 py-2">{exp.concept}</td>
                                    <td className="px-4 py-2 text-right">{formatCurrency(exp.baseAmount)}</td>
                                    <td className="px-4 py-2 text-right">{formatCurrency(getCuotaIVA(exp.baseAmount, exp.vatRate))} ({exp.vatRate}%)</td>
                                    <td className="px-4 py-2 text-right font-bold">{formatCurrency(getTotalFacturaRecibida(exp))}</td>
                                    <td className="px-4 py-2 text-center">{ (exp.isDeductible ?? true) ? 'Sí' : 'No' }</td>
                                    <td className="px-4 py-2 flex justify-center gap-1">
                                        <Button size="sm" variant="ghost" onClick={() => onEditExpense(exp)} title="Editar"><Icon name="pencil" className="w-4 h-4" /></Button>
                                        <Button size="sm" variant="ghost" onClick={() => onDelete('expense', exp.id)} title="Eliminar"><Icon name="trash" className="w-4 h-4 text-red-500" /></Button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    ...
                    <table className="w-full text-sm">
                         <thead className="text-xs text-slate-700 uppercase bg-slate-50 dark:bg-slate-700 dark:text-slate-400">
                            <tr>
                                ...
                                <th className="px-4 py-2 text-right">Valor Adquisición</th>
                        ...
                        <tbody>
                            {investmentGoods.sort((a,b) => new Date(b.purchaseDate).getTime() - new Date(a.purchaseDate).getTime()).map(good => (
                                <tr key={good.id} className="border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600">
                                    <td className="px-4 py-2">{formatDate(good.purchaseDate)}</td>
                                    <td className="px-4 py-2">{good.description}</td>
                                    <td className="px-4 py-2 text-right">{formatCurrency(good.acquisitionValue)}</td>
                                    <td className="px-4 py-2 text-center">{good.usefulLife} años</td>
                                    <td className="px-4 py-2 text-right font-bold">{formatCurrency(good.acquisitionValue / good.usefulLife)}</td>
                                    <td className="px-4 py-2 flex justify-center gap-1">
                                        <Button size="sm" variant="ghost" onClick={() => onEditInvestment(good)} title="Editar"><Icon name="pencil" className="w-4 h-4" /></Button>
                                        <Button size="sm" variant="ghost" onClick={() => onDelete('investment', good.id)} title="Eliminar"><Icon name="trash" className="w-4 h-4 text-red-500" /></Button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
...
const MisImpuestosView: React.FC = () => {
    ...
    return <Card><h3 className="text-xl font-bold">Mis Impuestos</h3><p>Funcionalidad en desarrollo.</p></Card>
}

const CalculadoraRentaView: React.FC = () => {
    // Placeholder component
    return <Card><h3 className="text-xl font-bold">Calculadora Renta Anual</h3><p>Funcionalidad en desarrollo.</p></Card>
}
    ...
    return (
        <Modal isOpen={isOpen} onClose={handleClose} title="Importar Factura con IA">
            <div className="space-y-4">
                ...
                <Select label="Tipo de Factura" value={invoiceType} onChange={(e) => setInvoiceType(e.target.value as 'income' | 'expense')}>
                    <option value="income">Emitida (Ingreso)</option>
                    <option value="expense">Recibida (Gasto)</option>
                </Select>
                <div>
                ...
                <div className="pt-4">
                    <Button onClick={handleAnalyze} disabled={isLoading || !file} className="w-full">
                        {isLoading ? 'Analizando...' : <><Icon name="sparkles" className="w-5 h-5" /> Analizar Factura</>}
                    </Button>
                </div>
            </div>
        </Modal>
